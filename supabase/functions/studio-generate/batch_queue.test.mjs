...
test("Studio.dc.html: kontrollü kuyruk motoru + kararlı opId + kalıcılık", () => {
  ...
  assert.ok(studioSrc.includes("(b.id + sc.key)") && studioSrc.includes("makeImage(sc.prompt, sc.idx, sc.key, b.aspect, opId, this._provResolve(provider), (b.style || undefined))"), "sahne başına kararlı opId + batch başında dondurulan ayarlar (sahne override resolver içinde önceliklidir)");
